...
- ✅ **域名白名单管理**：默认只在 localhost 和 127.0.0.1 运行，其他网站需手动添加到白名单
...
- **Plasmo 中文文档**：https://www.plasmocn.org/zh
- **Plasmo 官方文档**：https://www.plasmo.com/
- **Plasmo GitHub**：https://github.com/PlasmoHQ/plasmo
- **LaunchExt 开发平台**：https://www.launchext.com/zh（基于 Next.js 15 + Plasmo 的完整 Chrome 扩展开发平台）
...
**插件默认只在 `localhost` 和 `127.0.0.1` 上运行**，其他网站需要先添加到白名单：

#### 方式一：通过弹窗快速添加

1. 访问需要使用的网站
2. 点击浏览器工具栏中的扩展图标
3. 在"当前网站"区域，点击开关将当前网站添加到白名单

#### 方式二：通过选项页面管理

1. 右键点击扩展图标，选择"选项"
2. 在"域名白名单管理"区域，输入域名并点击"添加"
3. 可以查看和管理所有已添加到白名单的域名

### 2. 使用自动填充功能

1. 打开包含表单的网页（确保该网站已在白名单中）
...
1. **域名白名单**：插件默认只在 `localhost` 和 `127.0.0.1` 上运行，其他网站需要手动添加到白名单才能使用。这是为了安全考虑，避免插件在未授权的网站上运行
2. **API 费用**：使用科大讯飞 API 会产生费用，请注意控制使用量。推荐使用 `lite` 模型，性价比最高
3. **数据安全**：API 密钥仅存储在浏览器本地，不会上传到任何服务器，不会发送到后台，完全本地化处理
4. **字段匹配**：插件通过字段的 name 或 id 属性匹配数据，确保表单字段有正确的 name 或 id
5. **网络要求**：需要能够访问科大讯飞 API 服务器（https://spark-api-open.xf-yun.com）
6. **弹窗识别**：插件会自动识别页面上的弹窗，优先填充弹窗内的表单。如果弹窗没有正确识别，可以尝试关闭弹窗后重新打开
7. **框架兼容**：插件支持原生表单元素和 Ant Design 组件，其他框架的组件可能需要额外适配
...
1. **域名未添加到白名单**：插件默认只在 `localhost` 和 `127.0.0.1` 上运行，其他网站需要先添加到白名单
2. **页面没有表单元素**：检查页面是否有表单元素（input、textarea、select），插件会自动检测并显示按钮
3. **页面动态加载**：如果页面是动态加载的，可能需要等待页面加载完成

### Q: 如何将网站添加到白名单？

A: 有两种方式：

1. **通过弹窗**：访问网站后，点击扩展图标，在"当前网站"区域点击开关添加到白名单
2. **通过选项页面**：右键点击扩展图标，选择"选项"，在"域名白名单管理"区域输入域名并添加
